...
if __name__ == "__main__":
    ...
    system_prompt = f"""
        ...
        - MANDATORY CUSTOMER DETAILS: If the user asks for "top customers" or any customer-related aggregation (e.g., "top 5 customers", "customers by revenue"), you MUST ALWAYS include BOTH:
          ...
          - CUSTOMER CONTEXT RULE: Whenever the user's prompt asks for individual 'customer' details (including phrases like "top customers", "get customers", "customer revenue", etc.), you MUST ABSOLUTELY ensure that the final result includes the customer's First_Name and Last_Name. However, EXCEPTION: If the user is strictly asking for an aggregated count by a dimension (e.g., "location with highest customer base", "count of customers per city"), DO NOT include individual First_Name and Last_Name, as that would break the aggregation. In those cases, just return the dimension and the count.
        - MANDATORY CUSTOMER DETAILS: If the user asks for specific individuals like "top 5 customers by revenue", you MUST ALWAYS include BOTH Postgres_Sales_DB and Mongo_Customer_DB.Customer to get First_Name and Last_Name. NEVER return only customer_id and revenue without customer names. BUT, if the user asks for aggregated geographic/categorical metrics like "location with highest customer base", you should ONLY aggregate on the location and count, and SKIP fetching individual customer names.
